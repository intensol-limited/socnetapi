...
describe Socnetapi::BloggerApi do
  before do
    @config = YAML::load(File.open(File.join(File.dirname(__FILE__), "config.yml")))["blogger"]
    @blogger = Socnetapi::BloggerApi.new(:api_key => @config["api_key"],
                                         :api_secret => @config["api_secret"],
                                         :token => @config["token"])
  end
  
  it "should connect to blogger" do
    @blogger.client.should_not be_nil
  end
